...
export const IconChoise = ({ icon, onSelectIcon }: IconChoiseProps) => {
   ...
   {IconVariants.map((iconVariant) => (
                <TouchableScale activeScale={1.2} activeOpacity={1}  key={iconVariant.name} style={[styles.button, icon === iconVariant.name && styles.active]} onPress={() => onSelectIcon(iconVariant.name)}>
                    {iconVariant.component(icon === iconVariant.name ? 'white' : 'black')}
                </TouchableScale>
            ))}
            </View>
